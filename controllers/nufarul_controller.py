...
Химчистка: заказы, услуги, статусы, печать штрихкодов, фото (BLOB).
"""
...
class NufarulController:
                ...
                sql = """
                    SELECT * FROM (
                        SELECT o.ID, o.ORDER_NUMBER, o.BARCODE, o.CLIENT_NAME, o.CLIENT_PHONE,
                               o.STATUS_ID, o.STATUS_CODE, o.STATUS_NAME,
                               o.TOTAL_AMOUNT, o.NOTES, o.CREATED_AT
                        FROM V_NUF_ORDERS_BLOCKCHAIN o
                        WHERE 1=1
                """
                params = {"lim": limit}
    ...
    def get_order_by_id(order_id: int) -> Dict[str, Any]:
        try:
            with DatabaseModel() as db:
                ...
                # Позиции заказа (blockchain)
                r2 = db.execute_query(
                    """SELECT i.ID, i.SERVICE_ID, i.SERVICE_NAME, i.QTY, i.PRICE, i.AMOUNT, i.NOTES
                       FROM V_NUF_ORDER_ITEMS_BLOCKCHAIN i
    ...
    def get_order_by_barcode(barcode: str) -> Dict[str, Any]:
        try:
            with DatabaseModel() as db:
                r = db.execute_query(
                    "SELECT ID FROM NUF_ORDERS_LEDGER WHERE BARCODE = :barcode",
    ...
    # ---------- AI search backend toggle ----------

    @staticmethod
    def ai_parse_order_oracle(text: str, threshold: int = 40) -> list:
        """Парсинг текста через Oracle PL/SQL пакет NUF_AI_SEARCH (вектор/fuzzy)."""
        try:
            with DatabaseModel() as db:
                r = db.execute_query(
                    """SELECT service_id, service_name, service_name_ro, price, unit,
                              qty, original, confidence
                       FROM TABLE(NUF_AI_SEARCH.parse_order(:txt, :thresh))""",
                    {"txt": text, "thresh": threshold},
                )
                rows = _norm_rows(r)
                return [
                    {
                        ...
                        "service_name_ro": row.get("service_name_ro"),
                        "qty": row.get("qty", 1),
    ...
    def update_order_status(order_id: int, status_id: int) -> Dict[str, Any]:
        """Запись изменения статуса в blockchain-лог (INSERT ONLY, неизменяемый)."""
        try:
            with DatabaseModel() as db:
                # Получаем текущий статус
                r_old = db.execute_query(
                    "SELECT STATUS_ID FROM V_NUF_ORDERS_BLOCKCHAIN WHERE ID = :id",
                    {"id": order_id},
                )
                old_status = r_old["data"][0][0] if r_old.get("data") and r_old["data"][0] else None
                # Записываем изменение в blockchain-лог
                db.execute_query(
                    """INSERT INTO NUF_ORDER_STATUS_LOG (ORDER_ID, OLD_STATUS_ID, NEW_STATUS_ID)
                       VALUES (:oid, :old_sid, :new_sid)""",
                    {"oid": order_id, "old_sid": old_status, "new_sid": status_id},
                )
                db.connection.commit()
            return {"success": True}
        except Exception as e:
    ...
    def create_order(
        ...
        try:
            with DatabaseModel() as db:
                ...
                # Получаем ID из sequence для blockchain-таблицы
                ...
                db.execute_query(
                    """INSERT INTO NUF_ORDERS_LEDGER (ID, ORDER_NUMBER, BARCODE, CLIENT_NAME, CLIENT_PHONE, STATUS_ID, TOTAL_AMOUNT, NOTES)
                ...
                for it in items:
                    ...
                    db.execute_query(
                        """INSERT INTO NUF_ORDER_ITEMS_LEDGER (ID, ORDER_ID, SERVICE_ID, QTY, PRICE, AMOUNT)
                           VALUES (:iid, :oid, :sid, :qty, :price, :amount)""",
                        {"iid": iid, "oid": order_id, "sid": service_id, "qty": qty, "price": price, "amount": amount},
                    )
        ...
        try:
            with DatabaseModel() as db:
                ...
                try:
                    ...
                    # Parse ready_date if provided (ISO format: YYYY-MM-DDTHH:MM)
                    rdate = None
                    if ready_date:
                        try:
    ...
    @staticmethod
    def save_photo(order_id: int, item_id: Optional[int], photo_base64: str,
                   mime_type: str = "image/jpeg", photo_name: Optional[str] = None) -> Dict[str, Any]:
        """Сохраняет фото (base64) как BLOB в NUF_ORDER_PHOTOS."""
        try:
            photo_bytes = base64.b64decode(photo_base64)
            photo_size = len(photo_bytes)
            with DatabaseModel() as db:
                cursor = db.connection.cursor()
                cursor.setinputsizes(photo_blob=oracledb.DB_TYPE_BLOB)
                cursor.execute(
                    """INSERT INTO NUF_ORDER_PHOTOS (ORDER_ID, ITEM_ID, PHOTO_BLOB, PHOTO_MIME, PHOTO_SIZE, PHOTO_NAME)
                       VALUES (:order_id, :item_id, :photo_blob, :mime, :psize, :pname)""",
                    {
                        "order_id": order_id,
                        "item_id": item_id,
                ...
                r = db.execute_query(
                    "SELECT MAX(ID) FROM NUF_ORDER_PHOTOS WHERE ORDER_ID = :oid",
                    {"oid": order_id},
                )
                photo_id = r["data"][0][0] if r.get("data") and r["data"][0] else None
                db.connection.commit()
                return {"success": True, "photo_id": photo_id, "photo_size": photo_size}
        except Exception as e:
            return {"success": False, "error": str(e)}

    @staticmethod
    def get_order_photos(order_id: int, include_data: bool = False) -> Dict[str, Any]:
        """Список фото заказа. include_data=True — возвращает base64 данные."""
        try:
            with DatabaseModel() as db:
                if include_data:
                    sql = """SELECT ID, ORDER_ID, ITEM_ID, PHOTO_BLOB, PHOTO_MIME,
                                    PHOTO_SIZE, PHOTO_NAME, CREATED_AT
                             FROM NUF_ORDER_PHOTOS WHERE ORDER_ID = :oid ORDER BY CREATED_AT"""
                else:
                    sql = """SELECT ID, ORDER_ID, ITEM_ID, PHOTO_MIME,
                                    PHOTO_SIZE, PHOTO_NAME, CREATED_AT
                             FROM NUF_ORDER_PHOTOS WHERE ORDER_ID = :oid ORDER BY CREATED_AT"""
                r = db.execute_query(sql, {"oid": order_id})
                return {"success": True, "data": _norm_rows(r)}
        except Exception as e:
            return {"success": False, "error": str(e), "data": []}
    ...
    def get_photo(photo_id: int) -> Dict[str, Any]:
        """Одно фото с полными base64 данными."""
        try:
            with DatabaseModel() as db:
                r = db.execute_query(
                    """SELECT ID, ORDER_ID, ITEM_ID, PHOTO_BLOB, PHOTO_MIME,
                              PHOTO_SIZE, PHOTO_NAME, CREATED_AT
                       FROM NUF_ORDER_PHOTOS WHERE ID = :pid""",
                    {"pid": photo_id},
                )
                rows = _norm_rows(r)
                if not rows:
    ...
    def delete_photo(photo_id: int) -> Dict[str, Any]:
        ...
        try:
            with DatabaseModel() as db:
                db.execute_query(
                    "DELETE FROM NUF_ORDER_PHOTOS WHERE ID = :pid",
                    {"pid": photo_id},
                )
                db.connection.commit()
                return {"success": True}
