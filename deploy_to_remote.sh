...
$SSH_CMD "$REMOTE_USER@$REMOTE_HOST" << EOF
    ...
    WALLET_BAK_DIR="/tmp/wallet.artgranit.\$(date +%s).bak"
    REL_WALLET_DIR=""
    if [ -f "$REMOTE_PATH/.env" ]; then
        cp "$REMOTE_PATH/.env" "\$ENV_BAK" && echo "  Сохранён .env (wallet не трогаем)"
        REL_WALLET_DIR=\$(grep '^WALLET_DIR=' "$REMOTE_PATH/.env" | cut -d'=' -f2- | tr -d '"' | tr -d "'" || true)
        if [ -n "\$REL_WALLET_DIR" ]; then
            case "\$REL_WALLET_DIR" in
                /*)
                    echo "  Wallet указан абсолютным путём: \$REL_WALLET_DIR"
                    ;;
                *)
                    if [ -d "$REMOTE_PATH/\$REL_WALLET_DIR" ]; then
                        mkdir -p "\$WALLET_BAK_DIR"
                        cp -R "$REMOTE_PATH/\$REL_WALLET_DIR" "\$WALLET_BAK_DIR/" && echo "  Сохранён относительный wallet: \$REL_WALLET_DIR"
                    fi
                    ;;
            esac
        fi
    ...
    if [ -n "\$REL_WALLET_DIR" ] && [ -d "\$WALLET_BAK_DIR/\$(basename \"\$REL_WALLET_DIR\")" ]; then
        mkdir -p "$REMOTE_PATH/\$(dirname \"\$REL_WALLET_DIR\")"
        cp -R "\$WALLET_BAK_DIR/\$(basename \"\$REL_WALLET_DIR\")" "$REMOTE_PATH/\$(dirname \"\$REL_WALLET_DIR\")/" && echo "  ✓ Восстановлен относительный wallet: \$REL_WALLET_DIR"
        rm -rf "\$WALLET_BAK_DIR"
    fi
...
# Шаг 6: Oracle deploy на remote по умолчанию отключён (БД общая с локальной)
echo "Шаг 6: Oracle-объекты на remote — пропуск (по умолчанию)..."
if [ "${DEPLOY_ORACLE_ON_REMOTE:-0}" = "1" ]; then
  echo "  DEPLOY_ORACLE_ON_REMOTE=1 → выполняем deploy_oracle_objects.py"
  $SSH_CMD "$REMOTE_USER@$REMOTE_HOST" "bash -s" << REMOTEEOF
    cd $REMOTE_PATH
    if venv/bin/python3 deploy_oracle_objects.py 2>/dev/null; then
      echo '  ✓ Oracle-объекты развёрнуты'
    else
      echo '  ⚠ deploy_oracle_objects.py не выполнен — проверьте .env и доступ к БД'
    fi
REMOTEEOF
else
  echo "  ✓ Пропущено (используйте DEPLOY_ORACLE_ON_REMOTE=1 только если это действительно нужно)"
fi
...
echo "=========================================="
echo ""
echo "Проект: $REMOTE_PATH | .env сохранён; relative wallet backup/restore выполнен при необходимости."
echo ""
