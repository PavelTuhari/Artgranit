...
Поэтому автором ставится служебный пользователь UNA из настройки контура
`UNA_USERID` — в этой базе такой есть, «Automat». Он выставляется через
`SET_ENV` и попадает в документ; если настройка пуста, автор остаётся
пустым, а имя пользователя портала пишется в примечание документа. Ответ
всегда сообщает, каким из двух путей пошло дело.
...
def create_document(sysfid: int, date: str, *, valuta: str = "LEI",
    ...
    with Biro26DB() as db:
        ...
        # Строку-спутник заводит сама учётная система: TRIG_AFTINS_TMDB_DOCS2
        # вставляет её сразу после заголовка, вместе с FIRST_USERID и
        # признаками копирования. Вторая вставка ломается об ORA-00001,
        # поэтому примечание дописывается правкой уже готовой строки.
        statements.append({
            "sql": "UPDATE TMDB_DOCS_ADD SET TXTCOMMENT = :note "
                   "WHERE COD = :cod",
    ...
    return _done({"cod": cod, "sysfid": int(sysfid), "date": date,
                  "userid": author},
                 f"автор документа: служебный пользователь UNA {author}"
