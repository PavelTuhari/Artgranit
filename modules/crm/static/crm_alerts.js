...
(function () {
  ...
  function renderCfg() {
    ...
    document.getElementById('al-cfg').innerHTML = `<h3 style="margin:0;padding:10px 12px;border-bottom:1px solid var(--line);font-size:14px">${esc(t('settings'))}</h3>
      ...
      <label>${esc(t('kinds'))}</label><div style="display:grid;grid-template-columns:1fr 1fr;gap:2px 10px">${kinds.map(k => `<label style="font-weight:400;font-size:12px"><input type="checkbox" class="ac-kind" value="${k}" ${(!on.length || on.includes(k)) ? 'checked' : ''}> ${esc(kindName(k))}</label>`).join('')}</div>
