...
class AutoparkController:
    ...
    def gps_replay(payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        та же gps.interpolate_route, что и live-симуляция, но геометрия
        для отрисовки (прямые между точками маршрута) масштабируется к
        дорожной длине через gps.road_scaled_track ПЕРЕД сохранением --
        сама по себе прямая систематически короче норматива NORM_KM (см.
        docs/Autopark/GPS_INTEGRATION.md, "дорожный коэффициент"), а
        FLT_TRIPS.FACT_KM/AutoparkController.gps_track считают длину как
        haversine-сумму именно СОХРАНЁННЫХ точек -- значит, дорожную
        длину нужно заложить в сами координаты, а не только в отчёт.
        Затем -- обычный шум ±150 м на узел (разброс GPS-приёмника).
        Пишет факт (apply_track_fact), поэтому только для рейса не в
        DRAFT (иначе он ещё не основание для факта, ТЗ п.6).
        """
        try:
            trip_id = _as_int(payload.get("trip_id"), "trip_id")
        except AutoparkValidationError as exc:
