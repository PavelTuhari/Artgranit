...
<section class="slide">
  ...
  {livebar([('Открыть модуль планограмм', f'{MODULE}#overview', ''),
            ('Контур эксплуатации сети', TBC, 'blue')],
           'магазины сети — из таблицы TBC_STORES работающей системы')}
  <div class="num">X</div>
</section>
...
<section class="slide">
  ...
  <p>Ничего не выключается и не мигрирует одномоментно. Новая система
     набирает данные параллельно, пока сеть работает как обычно.</p>
  <div class="steps" style="margin-top:12px">
    <div class="step">
      <h3>Неделя 1 · Super Bonus Мунчешть</h3>
      <p>Самый крупный формат сети. Выгружаем год продаж в тестовый набор,
         прогоняем backtest всех четырёх моделей — получаем честную точность
         на реальном спросе сети, а не на демо-данных.</p>
    </div>
    <div class="step">
      <h3>Неделя 2 · одна товарная группа</h3>
      <p>Заводим реальных поставщиков и договоры по группе, размечаем зоны зала
         Bonus Дачия, собираем первый мониторинг цен по 100 SKU против Linella
         и Nr.1.</p>
    </div>
    <div class="step">
      <h3>Неделя 3 · заказ и график завоза</h3>
      <p>Формируем рекомендуемый заказ по категории и сравниваем с тем, что
         заказал бы закупщик. Ставим расписание завоза на диаграмму Ганта.</p>
    </div>
    <div class="step">
      <h3>Неделя 4 · решение</h3>
      <p>Смотрим на цифры: расхождение заказа, найденные ценовые перекосы,
         сорванные окна разгрузки. Решение принимается по результату,
         а не по презентации.</p>
    </div>
  </div>
  <div class="card accent">
    <h3>Что нужно от сети на старте</h3>
    <p style="font-size:14.5px">Выгрузка продаж одного магазина за год и решение,
       какая товарная группа берётся первой. Всё остальное уже стоит и работает.</p>
  </div>
  {livebar([('Открыть систему', f'{MODULE}#overview', ''),
            ('Генератор тестовых наборов', f'{MODULE}#testdata', 'blue'),
