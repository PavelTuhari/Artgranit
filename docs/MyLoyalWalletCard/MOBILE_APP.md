...
### Что нашла и починила сквозная проверка 12.09.2026

Прошёл все экраны на эмуляторе в двух темах и трёх языках. Найденное:

| Дефект | Исправление |
|---|---|
| Русские числительные: «2 товаров», «3 позиций», «1 изменений» | Множественные формы в словарях: `<ключ>.one/.few/.many/.other`, выбор формы по языку (`pluralCategory` в `src/i18n`) — ru: one/few/many, ro: one/few/other, en: one/other |
| Способ оплаты в чеке печатался кодом: «Оплата: card» | `receipt.payment_method.card` → «картой» (`src/ui/labels.ts`, неизвестный код печатается как есть) |
| Единицы измерения из данных: «1 l × …», «1 buc» | `unit.l`, `unit.kg`, `unit.buc` → «л», «кг», «шт» |
| Число товаров в категории выводилось голой цифрой | `category.product_count` с формами |
| Не загрузившаяся картинка оставляла пустое серое пятно на пол-экрана | `RemoteImage`: при отсутствии URL или ошибке загрузки — иконка на нейтральной подложке |
| На вкладке «Карта» у гостя был текст «Войдите, чтобы увидеть карту» без кнопки входа | Кнопка «Войти» в том же блоке |
| «Последняя синхронизация: Никогда» с зелёной галочкой | Нейтральный тон и иконка облака, пока синхронизации не было |
| Ключи, собираемые в коде (`account.<поле>`, `locale.<код>`, `sort.<ключ>`, дни недели), проверка пропускала | Валидатор выводит их из конфигурации и констант (`DAY_ORDER`, `SORT_KEYS`) и требует наличия в каждом словаре |

Проверка множественных форм покрыта тестами (`__tests__/domain.test.ts`), словари
сверяются по группам: язык обязан иметь ровно те формы, которые сам использует.

⚠️ **В dev-режиме правка словаря не подхватывается Fast Refresh** — JSON остаётся
в бандле старым. После изменения `config/l10n/*.json` приложение надо перезапустить
(`adb shell am force-stop md.una.retail` и запустить снова). На проде словарь
приходит из `GET /app-config` и применяется после перезапуска приложения.
