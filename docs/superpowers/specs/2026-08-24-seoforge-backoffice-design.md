...
## 9а. Перенос в боевую ERP (25.08.2026)

Решение §2.1 (контур в облачной БД бэкофиса) **отменено по указанию
заказчика**: контур перенесён в боевую ERP OfficePlus/UNA
(`orange.una.md:4024/cloudbd.world`, схема `OFFICEPLUS`, Oracle 11.2.0.4).

Что изменилось по факту переноса:

| Было | Стало |
|---|---|
| Облачная БД бэкофиса, thin-режим, `models/database.py` | Боевая ERP, thick-режим, `models/biro26_db.py` |
| `deploy_oracle_objects.py --only yseo` | `scripts/seoforge_deploy_erp.py --yes` |
| Многокомандная операция: несколько `execute_query` + `commit()` | Одна операция = один `execute_script` (постоянного соединения нет) |
| — | `scripts/seoforge_drop_cloud.py` — снятие копии с облачной БД |

Аргументы §2.1 против установки в ERP сняты не были: состав
`TMDB_DOCS_ADD`/`TMDB_DOCS_LOG`, выдача `TMDB_DOCS.COD`, кодировка
`DOCCOLOR` и сигнатура `UN$GFC` по-прежнему не подтверждены командой UNA.
Поэтому контур лежит в схеме ERP **автономно**: внешних ключей на
`TMS_UNIVERS` и `TMDB_DOCS` он не заводит, `YSEO_XREF` остаётся точкой
будущей связки. Это ограничивает риск переноса добавлением новых объектов
с собственным префиксом — существующие объекты ERP не затронуты.

Проверено на боевой базе: установка 77 команд без ошибок, все объекты
валидны, живой smoke — 22 проверки из 22, кириллица в `CL8MSWIN1251`
читается верно.

## 10. Что остаётся открытым

- Кусок C (документы и согласование): технический блокер **снят 25.08.2026**
  сверкой прямо в боевой схеме `OFFICEPLUS` — результаты в
  [`docs/SEOForge/UNA_INTERFACE.md`](../../SEOForge/UNA_INTERFACE.md).
  Три предположения ТЗ из четырёх не подтвердились: колонок
  `CAMP_CODE`/`RUN_ID`/`PLAYBOOK_SHA`/`NOTE` в `TMDB_DOCS_ADD` нет,
  `TMDB_DOCS_LOG` — журнал «старое/новое», ведомый триггером, а `DOCCOLOR` —
  `VARCHAR2(1)` со смыслом «пересчитан / есть проводки», а не статус
  согласования. `TMDB_DOCS.COD` выдаёт `ID_TMDB_DOCS.NEXTVAL` из триггера,
  `MAX(COD)+1` недопустим. `UN$GFC` подтверждён:
  `setDoc_GFC(nrdoc)` + `setDoc_Correct(nrdoc)`.
- Остались **не факты, а решения владельца учёта**: значения
  `TIP`/`SYSFID`/`AT1` для маркетинговых документов (`TIPDOC` в этой базе
  пуст у всех), выбор нумератора `NRMANUAL` (`UN$G$UTIL.TextIncrement` из ТЗ
  в схеме отсутствует, у каждого контура свой), рабочий план счетов и
