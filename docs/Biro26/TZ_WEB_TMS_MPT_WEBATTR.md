...
> **Статус БД:** все объекты созданы и заполнены в продакшене; данные set 8 загружены —
> **29 663** товара, из них **18 877** с описанием (см. §8a: привязка описаний исправлена).
...
Текущее наполнение: **29 663** строки, из них **18 877** с описанием (RO — из файлов поставщиков;
...
## 8a. ✅ Исправлено: чужие описания у 1 274 товаров (ваша находка)

**Спасибо за находку — это была ошибка на стороне импорта, уже устранена.**

**Причина** (не сдвиг строк): в файле `ULTRA.md` **один и тот же артикул присвоен разным
товарам на разных листах** — ошибка данных поставщика. Пример `GOG011050`:

| Лист файла | Название в файле |
|---|---|
| `Smartphone`, строка 1050 | Smartphone Xiaomi 12T Pro , 8 GB / 256GB |
| `Imprimare si Scanere`, строка 50 | Imprimanta laser HP LaserJet Pro 3003dn |

Импорт обрабатывал листы по очереди, и **последний перезаписывал** описание. В каталоге
`GOG011050` = Xiaomi, поэтому на витрине оказался принтер.
Масштаб конфликта в set 8: **1 908** артикулов / 3 929 строк.

**Что исправлено в пакете `BIRO26PT_importData`:** при записи в `TMS_MPT_WEBATTR` строка
выбирается **с проверкой названия по каталогу** — берётся та, чьё `DENUMIRE` совпадает с
`TMS_UNIVERS.DENUMIREA`; если ни одна не совпадает, описание **не пишется вовсе**
(лучше пусто, чем чужое):

```sql
ROW_NUMBER() OVER (PARTITION BY s.cod_univers
  ORDER BY CASE WHEN UPPER(TRIM(SUBSTR(s.denumire,1,60))) = UPPER(TRIM(SUBSTR(u.denumirea,1,60))) THEN 0
                WHEN UPPER(TRIM(SUBSTR(s.denumire,1,25))) = UPPER(TRIM(SUBSTR(u.denumirea,1,25))) THEN 1
                ELSE 2 END, s.id) rn
...
WHERE rn = 1 AND name_ok = 1     -- только надёжные совпадения
```

**Данные перезагружены.** Результат:

| Показатель | До | После |
|---|---|---|
| Товаров с **чужим** описанием | 1 274–1 778 | **0** |
| Строк в `TMS_MPT_WEBATTR` | 30 004 | 29 663 |
| Из них с описанием | 19 702 | 18 877 |

Уменьшение на 341 — это как раз конфликтные артикулы, для которых описание теперь **не
пишется** (вместо чужого). Проверьте на своём примере: `COD 303284` → теперь
`Smartphone Xiaomi 12T Pro , 8 GB / 256GB` в обеих таблицах.

**SQL-проверка** (можете использовать как регресс-тест):
```sql
-- должно вернуть 0 «чужих» описаний
SELECT COUNT(*) FROM tms_mpt_webattr w JOIN tms_univers u ON u.cod = w.cod
WHERE w.denumire_full_ro IS NOT NULL
  AND UPPER(TRIM(SUBSTR(w.denumire_full_ro,1,25))) <> UPPER(TRIM(SUBSTR(u.denumirea,1,25)));
```
Сейчас возвращает **39** — но это **не** чужие товары, а различия в написании символов
у одного и того же товара: типографские кавычки (`„Laba”` vs `„Laba"`) и латинская `x`
против кириллической `х` (`420x594` vs `420х594`). Если хотите строгий тест — сравнивайте
после нормализации кавычек и `x/х`.
...
7. **Триггер-защита может блокировать импорт.** `YBIRO_UNIVERS_CHK_DIACRITICE` не даёт
   записать в `TMS_UNIVERS` текст с испорченной диакритикой (`5?А`). Если импорт падает с
   `ORA-20077` — данные в исходном файле плохие: пусть поставщик перевыгрузит, либо
   временно `ALTER TRIGGER YBIRO_UNIVERS_CHK_DIACRITICE DISABLE` (осознанно).
8. `DESCRIERE_RU/EN` сейчас пустые — UI должен корректно показывать пустое состояние
