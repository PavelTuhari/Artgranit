...
### Ускорение каталога: 4–8 с → 0,7–1,5 с (2026-08-28)

Каталог отвечал за 3–8 с (поиск — до 14 с). Измерения показали четыре
независимые причины; исправлены все.

**1. Штрихкоды агрегировались по всей таблице.** В запросе каталога стоял
`LEFT JOIN (SELECT COD, MIN(BARCODE), COUNT(*) FROM TMS_MPT_BARCODE GROUP BY
COD)` — Oracle считал агрегат по **197 704 строкам** на каждый запрос, чтобы
отдать штрихкоды для 24 товаров страницы: **2,3–3,0 с**. Заменено скалярными
подзапросами, которые выполняются только для строк страницы и попадают в
готовый индекс `TMS_MPT_BARCODE_PK (COD, BARCODE)`: **0,04 с** — в 58 раз
быстрее. Так же переписан счётчик вариантов (`GROUP BY` по 59 693 строкам).

*Ловушка при переписывании:* `COUNT(*)` по `NULL`-ключу возвращает **0, а не
NULL**, поэтому `NVL(..., 1)` не срабатывал и у товаров без вариантов
`var_cnt` стал 0. Нужен явный `CASE WHEN vr.MASTER_COD IS NULL THEN 1`.

**2. Дерево групп и бренды считались заново на каждой странице** (~1,6 с),
хотя одинаковы для всех и меняются раз в сутки — после импорта. Добавлен
кеш с TTL, который сбрасывается в `prepare_input()` / `assign_keys()`.

**3. Кеш в памяти промахивался в половине случаев.** В производстве
приложение работает под **gunicorn с 2 воркерами** — у каждого своя память,
поэтому «холодный» воркер платил полную цену. Кеш сделан общим: значения
дублируются в файлы (`/tmp/biro26_cache`, атомарная запись через
`os.replace`), данные публичные — дерево групп, бренды, счётчики.

**4. Дерево весит ~400 КБ несжатыми** и передавалось через туннель
офис→облако при каждом переходе по каталогу. Публичные справочники теперь
отдаются с `Cache-Control: public, max-age=600` — браузер берёт их из
своего кеша.

**Поиск — отдельная история.** Он стоил 4–6 с, и 3,65 с из них — скан
**CLOB-описаний** (`DBMS_LOB.INSTR`): полнотекстового индекса нет, поэтому
Oracle читает все описания на каждый запрос. Пробовал пропускать описания,
когда есть совпадения по названиям — **отказался**: результат меняется
молча (по запросу «ergonomic» находилось 406 товаров, стало бы 143). Вместо
этого ответ поиска кешируется на 5 минут целиком: полнота сохранена, а цена
платится один раз — пагинация и следующие посетители получают готовое.

**Итог на боевом сайте** (страница каталога целиком — 1,5 с вместо 4–8 с):

| Запрос | Было | Стало |
|---|---|---|
| дерево групп | 1,6–2,6 с | 0,9 с, дальше из кеша браузера |
| каталог по группе | 2,5–4,5 с | 1,2–1,6 с |
| поиск | 8,7–14,5 с | 0,7 с |
| вторая страница поиска | 6,5–9,8 с | 0,6 с |

Проверено, что выдача не изменилась: штрихкоды, `var_cnt` и `total`
сверены с базой построчно.

**Что осталось и требует решения владельца.** Радикальное ускорение поиска
(первый, некешированный запрос — 2–3 с) упирается в отсутствие индекса
Oracle Text на описаниях. Это DDL на боевой БД плюс регулярная
синхронизация индекса — отдельная задача, не делал без согласования.

**Контекст витринных страниц.** Функция `_biro26_site_ctx()` собиралась заново
на КАЖДОЙ странице и делала 5–6 запросов в Oracle (тарифы рассрочки,
настройки, колонка цены) — на медленном контуре страница доходила до 8 с.
Теперь клиент-независимая часть кешируется 60 с **с ключом по домену** (важно:
`ga_id` для Google Analytics ставится только на публичном домене), а колонка
цены по типу клиента (физическое/юридическое лицо) вычисляется каждый раз —
иначе юрлицо увидело бы розничные цены. При старте приложения прогреваются
дерево, бренды и контекст. Главная страница на медленном контуре: 8,1 с →
**0,58 с**.

Итог по обоим контурам (страницы, не API):

| Страница | nufarul (медленный) | officeplus.md (боевой) |
|---|---|---|
| главная | 8,1 с → **0,58 с** | 1,0 с |
| каталог | ~8 с → **0,57 с** | 0,72 с |
| каталог с фильтром | — | 0,80 с |
| корзина | — | 0,88 с |

Кеш контекста и прогрев применены только в `main` (nufarul): на боевой
площадке страницы и так отвечают за 0,7–1,0 с, а её `app.py` принадлежит
другой ветке — лишний патч там не оправдан.

### Индекс Oracle Text: поиск стал мгновенным (2026-08-28, продолжение)

По согласованию с владельцем сделан полнотекстовый индекс — то, что раньше
было записано как «требует решения».

**Права.** Роли `CTXAPP` у пользователя приложения нет и пакет `CTXSYS.CTX_DDL`
ему не виден, но выяснилось, что **индекс с параметрами по умолчанию
создаётся и без этой роли**. Поэтому синхронизация выбрана `SYNC (ON COMMIT)`
— индекс обновляется сам после каждого коммита; периодический
`CTX_DDL.SYNC_INDEX` нам недоступен.
...
Создание — 4 секунды, размер 25 МБ на 18,6 МБ текста (49 276 описаний).
DDL: `sql/biro26/19_webattr_text_index.sql`, откат — `DROP INDEX`.

**Где индекс применяется — и где намеренно НЕ применяется.** `CONTAINS` ищет
слова, `DBMS_LOB.INSTR` — подстроку, и для многословных запросов это разная
семантика. Проверено:

| запрос | было (INSTR) | стало бы (CONTAINS) |
|---|---|---|
| одно слово (hp, toner, ergonomic, creion, plastic, birou, a4) | — | **множества COD идентичны** |
| «creion mecanic» | 3 | 14 |
| «50%» | 139 | 8 091 |

Поэтому индекс используется **только для однословных запросов без спецсимволов**
(`_one_word()`), где доказано полное совпадение; многословные и со знаками идут
прежним путём — медленнее, но с тем же результатом, что клиент видел раньше.
Если индекс когда-нибудь пропадёт, `_text_index_ok()` это заметит (проверка
кешируется на 10 минут) и поиск сам вернётся к сканированию.

**Второй недостающий индекс.** `BIRO26_GOODS` (201 212 строк) связывается с
`TMS_UNIVERS` по `COD_UNIVERS`, но индекса на этой колонке **не было вообще** —
соединение шло без него на каждой странице каталога. Индекс создан (0,4 с),
статистика пересобрана. Дубликаты в фиде реальны (3 631 повторяющийся
`COD_UNIVERS`), поэтому дедупликация `ROW_NUMBER` остаётся нужной.

**Найденный попутно дефект пагинации.** Сортировка шла только по `DENUMIREA`.
Товары с одинаковым названием Oracle волен вернуть в любом порядке, и при
смене плана выполнения страницы «разъезжались»: часть товаров пропадала,
часть дублировалась. Добавлен второй ключ `u.COD` во все варианты сортировки —
порядок теперь детерминирован. Проверено: три страницы подряд, 72 строки,
пересечений между страницами — ноль.

**Итог по поиску на боевом сайте:**

| | было | стало |
|---|---|---|
| поиск «hp» | 8,7–14,5 с | **0,62 с** |
| поиск «toner» | 5,0–7,8 с | **0,60 с** |
| поиск «creion» | 8,0–9,8 с | **0,54 с** |
| скан описаний (сам предикат) | 2,2–2,8 с | **0,02–0,18 с** |

Полнота выдачи сверена построчно на каждом шаге: состав товаров, `total`,
штрихкоды и `var_cnt` совпадают с прежними.

### Хаб проекта приведён в порядок + просмотрщик документации (2026-08-31)

**Хаб** `/UNA.md/orasldev/biro26` был плоской кучей из 24 плиток, где новые
возможности терялись, а часть опций отсутствовала вовсе. Теперь — **25 плиток
в пяти разделах**: Витрина · Бэк-офис · Интеграции · Отчёты и печатные формы ·
Документация. Сверху — быстрые ссылки (все модули портала, документация,
health с текущим коммитом, боевой сайт).

Добавлено то, чего в хабе не было: **e-Factura (SFS)**, **Partner API (B2B)**,
переводы групп, тест платежей MAIB, журнал интеграций, визуальный дизайнер PDF,
магазин-классика. У каждой плитки — ссылка на её документацию.

**Просмотрщик документации** (новый изолированный модуль `modules/b26docs/`).
Раньше `.md`-документы проекта нельзя было открыть из портала: они жили только
в репозитории, а хаб отдавал одну статичную HTML-страницу — ссылки «где это
документировано?» вели в пустоту. Теперь:

* `/UNA.md/orasldev/b26docs/` — индекс **31 документа** двух папок
  (`docs/Biro26`, `docs/Partner`) с поиском по названиям и описаниям;
* `/UNA.md/orasldev/b26docs/<папка>/<файл>` — markdown рендерится в HTML
  (таблицы, код, оглавление), HTML отдаётся как есть;
* читает **только** из списка разрешённых папок и только `.md`/`.html`;
  обход каталога (`../`) и посторонние расширения дают 404 — проверено;
* без сессии — переход на страницу входа, а не сухой 401.

**`docs/Biro26/docs.json`** — реестр из 28 документов: в индексе видны
человеческие названия и описания вместо имён файлов. Файл без записи в
реестре всё равно появляется (реестр сканирует папку), просто без описания.

*Ловушка при разработке:* ключ списка в шаблоне нельзя называть `items` —
в Jinja `g.items` берёт метод словаря, а не список, и страница падает с
