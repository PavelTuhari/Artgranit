...
### Компактные карточки каталога + ВАЖНО: где на самом деле живёт боевой officeplus.md (2026-08-27)

**Карточки товаров сделаны компактными** (требование владельца): сетка
`repeat(auto-fill, minmax(178px, 1fr))` вместо фиксированных колонок, картинка
160→118 px и `contain` вместо `cover`, название 16→13 px с обрезкой в 3 строки
(вместо `min-height: 42px`), цена 18→15.5 px, кнопка 42→32 px, отступы и тени
меньше, `min-height: 360px` у карточки убран совсем.

**Настоящая причина гигантских карточек была не в размерах.** В `site_base.html`
стояло `@media (min-width:1025px){.plp .product-grid{grid-template-columns:
repeat(4,1fr)}}`. Трек `1fr` — это `minmax(auto, 1fr)`, и он **не сжимается
ниже min-content**: длинные коды без пробелов (штрихкод + артикул, которые мы
вывели на карточки 13.08) раздували колонки — фактические ширины были
638/488/**909**/314 px при контейнере 924 px, то есть в ряд влезали две
карточки. Исправление: та же `auto-fill`-сетка + `.product-card{min-width:0}` и
`overflow-wrap:anywhere` для названия и кода. После: ровные колонки 222 px,
4 карточки в ряд, высота 317 px (было 636×315).

**Открытие про контур officeplus.md — учитывать при любом деплое.** Домен
обслуживает НЕ сервер 92.5.130.1 напрямую: `conf.d/officeplus-hybrid.conf`
проксирует всё на `127.0.0.1:8001`, а это `officeplus-failover.conf`, который
шлёт запрос в **офисную сеть 93.115.136.18** (машина `192.168.0.250`, доступна
через VPN93), и лишь при её недоступности падает на локальный Flask :8000
(запасная площадка). Проверка: заголовок `X-Shop-Origin: office | spare`.

Практические следствия:

1. деплой в `/home/ubuntu/artgranit` на 92.5.130.1 обновляет **только
   запасную** площадку — на боевом сайте изменений не видно;
2. на офисной площадке **своя, более свежая ветка** (1020 маршрутов против 753
   в `main`, коммит `b08c712…`; там, например, картинка и название карточки уже
   сделаны настоящими ссылками `<a>` для поисковиков). Заливать туда `main`
   целиком нельзя — затрёт чужую работу;
3. правки туда вносить **точечно**: скачать файл, применить изменение поверх
   их версии, залить обратно с резервной копией (`*.bak-*`), затем
   `sudo systemctl restart artgranit` (Jinja кеширует шаблоны — без рестарта
   правка в `site_base.html` не подхватится);
4. доступ: VPN93 + пароль из Keychain
   (`security find-internet-password -s 192.168.0.250 -a ubuntu -w`), sudo
   через `sudo -S`.

Сегодняшняя правка внесена во все три места: офисная (боевая) площадка,
запасная на 92.5.130.1 и nufarul.

### Мобильная версия «как у лидеров» (2026-08-28)

Новый слой `static/biro26/site-mobile.css` (+ правки в `site_base.html` и
`site.js`). Подключается **последним в `<head>`** — при равной специфичности
побеждает правило, идущее позже, а блок `<style>` внутри `site_base.html`
(десктопные размеры) иначе отменял мобильные.

**Что появилось:**

1. **Нижняя панель навигации** (`.bnav`, app-like, как в приложениях Amazon /
   eMAG / Wildberries): Acasă · Catalog · Coș · Favorite · Cont, с бейджем
   корзины и подсветкой текущего раздела. Иконки — инлайн-SVG, без запросов.
2. **Липкий заголовок** с поиском: `position: sticky`; верхняя служебная
   полоса и кнопка «Catalog» на телефоне скрыты (их роль взяла нижняя панель).
   Пришлось снять `overflow: hidden` у `.homepage` — иначе sticky не работает.
3. **Фильтры каталога — bottom-sheet**: выезжает снизу, со скруглением,
   «ручкой», крестиком, затемнением фона и блокировкой прокрутки страницы.
4. **Липкая панель покупки на карточке товара**: количество + «Adaugă în coș»
   всегда на экране.
5. **iPhone/Android**: `viewport-fit=cover` + `env(safe-area-inset-bottom)`
   (чёлка и home-indicator), `theme-color`, отключены синие вспышки при
   касании и зум при фокусе (шрифт полей 16px), тап-цели ≥ 40–44px.
6. **Карточки одной высоты**: название обрезается на 3 строки, код — на одну.

**Три ловушки, которые стоили отладки:**

* `1fr` = `minmax(auto, 1fr)` и **не сжимается ниже min-content**: длинные
  штрихкоды раздували каталог до 1460px внутри экрана 343px. Решение —
  `repeat(2, minmax(0, 1fr))` + `min-width: 0` контейнерам.
* `width` побеждает `right` у `position: fixed` — из-за `width: 280px` (блок
  `<style>`) шторка фильтров открывалась узкой; нужен `width: auto`.
* Убрав `min-height` у карточки (компактность 27.08), мы обнулили и
  карточки-скелетоны: во время загрузки страница выглядела пустой. Вернули
  им собственную высоту.

**Кеш статики.** Статика отдаётся с `max-age=604800`. В `main` ссылки
переведены на `asset()` (метка `?v=<mtime>`), на офисной площадке — с
`?v={{ asset_v }}` (метка коммита) тоже на `asset()`: при точечной правке
файла коммит не менялся, и браузеры неделю показывали старый CSS.

Раскатано на оба контура: nufarul (полный деплой) и боевая офисная площадка
192.168.0.250 (точечно, с резервными копиями `*.bak-mobile`, `*.bak-assetv`).
Проверено на 375×812 (iPhone) и 360×800 (Android): горизонтального
переполнения нет ни на одной странице.

**Замеченное попутно (не чинилось):** на nufarul запрос
`/api/biro26/shop/products?grupa=…` не укладывается в 60 с, тогда как без
фильтра отвечает за 3,4 с. На боевой площадке тот же запрос быстрый — похоже
на нехватку памяти слабой машины (956 МБ).

**Регресс от чужой перезаливки (28.08).** На офисной площадке `site_base.html`
был перезалит другой командой со своей ветки, и вчерашняя правка компактных
карточек (auto-fill 178px, `min-width: 0`, `overflow-wrap`) исчезла — на
десктопе снова вернулись карточки-гиганты. Правило перенесено из общего
шаблона в **наш** `site-mobile.css` (подключается последним и чужими
перезаливками не затрагивается). Общий вывод: всё, что должно пережить
перезаливку общего шаблона, держать в собственном файле.
