...
### Автоприсвоение NRMANUAL (y_ai_BIRO26.ensure_nrmanual)

Документы, созданные ДРУГИМИ приложениями, приходят без `NRMANUAL`, и
`gen_conturi_pr` раньше падал: `ORA-20000: ERR: documentul COD=307 nu are
NRMANUAL`. Теперь номер присваивается автоматически по тем же правилам, что
и при выписке счёта на сайте: серия `YBIRO_SETTINGS.INVOICE_SERIES` +
счётчик `INVOICE_NR_START`, при номере >999 серия сдвигается (A→B→…).

Ключевые решения (проверены на живой базе, см. sql/oracle_biro26/):

1. **Автономная транзакция + COMMIT сразу.** Если присваивать номер в
   транзакции вызывающего, строка документа остаётся заблокированной на
   время HTTP-запроса, а сайту (другая сессия) она нужна, чтобы прикрепить
   PDF, — взаимоблокировка через сеть: Oracle ждёт сокет, сайт ждёт строку.
2. **`SELECT … FOR UPDATE NOWAIT`.** Документ, открытый в другой сессии
   (нативное приложение другого пользователя), даёт мгновенное понятное
   сообщение вместо ожидания. Такой документ нужно сначала освободить.
3. **Обход триггера периода.** У документов чужих приложений `AT2 IS NULL`,
   а рабочий период в `TPARAMS` не заполнен, поэтому
   `TRIG_BFALL_TMDB_DOCS` запрещает любой UPDATE. Присвоение номера —
   системная операция, используется штатный обход самого триггера:
   `un4public.envun4.envsetvalue('dont_fire_trigger','1')` (им же пользуется
   импорт), флаг снимается сразу после UPDATE.
4. **`msg()` = `RAISE_APPLICATION_ERROR`** (UN4PUBLIC.MSG) — любой вызов
   ОБРЫВАЕТ процедуру. Поэтому отладочный `msg(v_url)` убран (он убивал
   вызов до HTTP-запроса), а итог сообщается только при ОШИБКЕ: успешная
   генерация завершается молча, без «ошибки» в вызывающем приложении.
5. **Обращение к сайту по внутреннему COD** (`?cod=`): номер `NRMANUAL`
   может совпадать с COD другого документа (напр. номер «307» принадлежит
   COD 177). Номер дополнительно передаётся в `&nr=` для печати.
6. На стороне сайта есть подстраховка `Biro26Report.assign_nrmanual()` с
   ограниченным ожиданием (25 с) — если Oracle вызвали без номера.

Проверка (на СВОИХ документах; чужие, открытые в приложении, не трогать):

```sql
BEGIN y_ai_BIRO26.gen_conturi_pr(318); END;   -- без номера -> A-71 + 2 PDF
BEGIN y_ai_BIRO26.gen_conturi_pr(327); END;   -- с номером  -> без изменений
```
