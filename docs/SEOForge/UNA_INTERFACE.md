...
1. ~~**Тип документа.**~~ **Закрыто** — см. раздел 8: тип задаёт `SYSFID`,
   равный свойству `DB ID` узла-документа в конфигурации uniConf. Диапазон
   `60000..60099` заведён под маркетинг, первые два типа созданы.
...
## 8. Конфигурация: журнал и типовые документы (сделано 25.08.2026)

Открытый вопрос «какой тип у маркетинговых документов» закрыт: тип задаёт
**`TMDB_DOCS.SYSFID`**, и он равен свойству **`DB ID`** узла-документа
в дереве конфигурации uniConf (`A$ADM$V`, `OBJ_TYPE=1`, `OBJ_SUBTYPE=0`).
Проверено сопоставлением: каждый `SYSFID` из данных ложится ровно на один
узел конфигурации. `TIPDOC` в этой базе пуст у всех документов и типом
не является.

### Как устроен журнал

Журнал — узел `OBJ_TYPE=2`, `OBJ_SUBTYPE=0`. Отбор документов задают:

| Свойство | Тип | Смысл |
|---|---|---|
| `SQLFILTER` | Memo | условие по `SYSFID` для выборки документов |
| `DOCTYPESFILTER` | Memo | условие по `id` для списка допустимых типов |
| `DOCTYPESDEFAULT` | Integer | тип по умолчанию для кнопки «новый документ» |
| `CAPTION`, `ACTIVE`, `DLL ID` | | заголовок, доступность, модуль |

### Что заведено

Диапазон `DB ID` **60000..60099** зарезервирован за маркетингом — перед
установкой он был свободен и в конфигурации, и в данных.

| Узел | Секция | OBJ_ID | Что это |
|---|---|---|---|
| Группа журналов | `J_SEO_MARKETING` | 11501 | «25. Маркетинг» |
| Журнал | `J_SEO_DOCS` | 11502 | «25.1 Документы маркетинга», фильтр `SYSFID 60000..60099` |
| Группа документов | `SEO_MARKETING` | 11503 | «Маркетинг» |
| Документ | `SEO_ACHIZ_RECLAMA` | 11504 | «Закупка рекламных услуг», `DB ID = 60001` |
| Документ | `SEO_ACT_PLASARE` | 11514 | «Акт о размещении рекламы», `DB ID = 60002` |

Установщик — `scripts/seoforge_erp_config.py` (`--dry-run` / `--yes`),
всё пишется одной транзакцией.

### Почему документы созданы копированием

У рабочего документа 47–52 свойства, 7–9 дочерних узлов (Actions,
PrintForm, Total) и 3–6 LOB — форма, гриды, печатные шаблоны. Собрать
такой набор с нуля и не промахнуться нельзя, а промах виден пользователю
как сломанная форма документа. Поэтому типовые документы скопированы
с ближайших по смыслу работающих:

- «Закупка рекламных услуг» ← `2:1:FS_48101` «Покупка услуг от фирм РМ»;
- «Акт о размещении рекламы» ← `2:0:FS_48106` «Акт закупки услуг».

Оба источника исполняет DLL `20` / `DOCNAME 201` — модуль закупки услуг.
Сверка после установки: расхождений в свойствах ноль, LOB перенесены
полностью (3/3 и 6/6). Изменены только имя, секция и `DB ID`.

Секции дочерних узлов получают префикс секции нового документа
(`SEO_ACT_PLASARE:2:11515` и т. п.): `SECTION` уникален по всему дереву,
и копия иначе столкнулась бы с оригиналом.

### Что осталось настроить

Копия унаследовала от источника проводки и счета закупки услуг. Под
маркетинг их нужно донастроить — это работа в самой конфигурации
(свойства документа и его Actions), а не в коде модуля:

- корреспонденция счетов под статьи маркетинга вместо счетов источника;
- нумерация `NRMANUAL` — какой нумератор берут маркетинговые документы;
- связь документа с кампанией контура через `YSEO_XREF`.
