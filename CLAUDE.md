...
6. **venv производственного сервера неприкосновенен.** 31.07.2026 сайт упал с 500, потому что сторонний процесс частично удалил файлы пакетов из `/home/ubuntu/artgranit/venv/` (`babel/locale-data/`, `jinja2/debug.py`, весь `markupsafe`). Запрещено удалять что-либо из `venv/` (включая «неиспользуемые» locale-данные и `__pycache__` в site-packages) и запускать `pip uninstall`/массовые «чистки» без проверки `curl -I https://nufarul.eminescu.md/login` → 200 сразу после. Детали и команда восстановления: `docs/NUFARUL_VENV_PROTECTED.md` (копии лежат на самом сервере: `DO_NOT_DELETE_VENV.md` в корне приложения и `venv/DO_NOT_CLEAN.md`).
