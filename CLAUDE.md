...
## Не писать важный код в общие файлы — правило №2

Владелец 31.08.2026, после того как параллельная сессия дважды затёрла
готовую работу: **«прекращай писать важный код в один и тот же файл, пиши в
разные файлы; каждое изменение документируй в md файл».**

Причина не в стиле, а в потерях. Дважды за две недели работа исчезла молча:

| Что | Куда писал | Чем кончилось |
|---|---|---|
| Порог 1500 лей для рассрочки | `app.py`, `site_cart.html`, `site.js`, `styles.css` | затёрто целиком, обнаружено случайно через 12 дней |
| Правила VPN и деплоя | `CLAUDE.md` | затёрто, восстановлено из коммита `bbc171e` |

Оба раза файл переписывала другая сессия целиком. Git тут не спасает:
конфликта нет, коммит «auto: update» проходит молча.

**Как писать теперь:**

1. **Логика — в отдельный файл модуля**, не в `app.py` и не в общий
   контроллер. Своя функция → свой файл `models/biro26_<тема>.py` или
   `modules/<ключ>/`. В общем файле остаётся только вызов в одну строку —
   его затереть сложнее, а если затрут, видно сразу.
2. **Стили и скрипты — в свой файл**, а не в конец `styles.css` / `site.js`.
   Подключать отдельным `<link>` / `<script>`.
3. **Настройки — в `YBIRO_SETTINGS`**, а не константой в коде. Тогда
   перезапись файла не уносит значение.
4. **Каждое изменение — в свой `.md`** в `docs/<Модуль>/`: что сделано,
   почему, какие файлы затронуты, как проверить. Файл документации сам
   становится страховкой: по нему работу можно восстановить.
5. **Перед правкой общего файла** — `git log --oneline -3 -- <файл>`.
   Если там свежие `auto: update` от другой сессии, свою логику туда не
   класть.

**Проверка, что работа не исчезла** (после чужих коммитов и перед сдачей):

```bash
grep -rn "<ключевое имя из твоей задачи>" --include=*.py --include=*.html --include=*.js . | head
```

Если пусто — работу затёрли, восстанавливать из своего коммита:
`git log --all --oneline -S"<имя>" -- <файл>`.

## Общий репозиторий: несколько сессий на одном дереве

Репозиторий один, а сессий и рабочих деревьев несколько (`git worktree list`).
Плюс в среде работает hook, который коммитит **каждую правку файла** —
в ту ветку, которая сейчас выкачана. Отсюда следствия, проверенные на
практике 25.08.2026:

1. **Чужая работа приезжает на твою ветку сама.** На `feat/sda-core-module`
   за день осело 15 коммитов `auto: update` с чужими файлами
   (`docs/OTRS/*.py`), плюс раньше — TBControl, MyLoyalWalletCard, SeoForge.
   Ничего из своего при этом не потерялось.
2. **Чужие коммиты не откатывать и не вырезать rebase'ом.** Это чья-то
   работа. Разделять по путям файлов: свой модуль — свои пути. Если нужен
   чистый набор, собирать его из своих коммитов, а не чистить ветку.
3. **Долго не сидеть на своей ветке в общем каталоге.** Чем дольше ветка
   выкачана в `/Users/pt/Projects.AI/Artgranit`, тем больше чужого на ней
   осядет. Довёл модуль до рабочего состояния — вливай в `main`.
4. **`git checkout main` может не сработать:** ветка бывает выкачана в чужом
   worktree, и git это запрещает. Для операций над `main` в таком случае
   заводить свой worktree (`git worktree add`), а не трогать чужой каталог.
5. **Перед первым коммитом смотреть, где ты:** `git branch --show-current`.
   Если это чужая feature-ветка — не коммитить туда, а завести свою.

Изоляция модулей (правило №1) как раз это и разруливает на уровне кода:
если модуль целиком лежит в `modules/<ключ>/`, чужие коммиты в общем дереве
физически не могут его задеть, а свои — не задевают чужого.
...
## Ничего не терять: изолированные файлы + push каждого шага — правило №3

Владелец 02.09.2026, после того как код mTLS-источника сервисов Zabbix,
Proxmox и мониторинга сертификатов TBControl (`models/zabbix_svc.py`,
`proxmox_svc.py`, `sql/79b–79e`, методы контроллера) пропал **без следа** —
не был закоммичен, прод перезалили поверх, транскрипты его не содержали:
**«так всё разрабатывай, чтобы никто не мог случайно стереть! и все push
commit на git!»**

1. **Логика — только в своём файле** (`models/<модуль>_<тема>.py`,
   `modules/<ключ>/`); в общих файлах — вызов в одну строку. Это правило №2,
   но теперь оно обязательно для любого кода, не только «важного».
2. **После каждого законченного шага — `git add` своих путей, `git commit`,
   `git push`** на текущую ветку. Не копить до конца задачи: hook коммитит
   «auto: update» только локально, без push работа живёт в одном каталоге.
3. **Перед сдачей** `git log origin/$(git branch --show-current)..HEAD` —
   должно быть пусто.
4. Каждое изменение — свой `.md` в `docs/<Модуль>/`: по нему работу можно
   восстановить, даже если код затёрли. Пример потери:
   `docs/TBControl/INDEX.md`, раздел 7.

## Не затирать чужие ветки и боевые площадки — правило №4

Владелец 05.09.2026, при задаче на документацию бэк-офиса: **«важно, что
ничего не затри из разработок в других ветках над этим же проектом»**.
Над проектом одновременно идут несколько веток (`git worktree list`), а
боевые площадки (например `192.168.0.250`) работают на **своём** наборе
файлов, который может быть новее любой ветки в репозитории — там уже
бывали чужие правки, которых в `main` нет.

1. **Перед правкой любого файла на площадке — сравнить его с локальным:**
   `md5sum` на сервере против `md5 -q` локально. Совпадает — можно класть
   свою версию. Не совпадает — на сервере чужая работа: скачать, внести
   свою правку **поверх их версии**, залить обратно с резервной копией
   (`cp файл файл.bak-ГГГГММДД`).
2. **Ни один файл на площадке не перезаписывается «из ветки целиком».**
   Никаких `git checkout <ветка> -- путь`, `rsync --delete`, распаковки
   архива поверх каталога, если в нём есть файлы, которых нет в архиве
   или которые новее.
3. **Новое — новым файлом.** Документация, скриншоты, стили, скрипты
   добавляются как отдельные файлы (правило №2); в общий файл попадает
   только ссылка или вызов в одну строку — точечным `Edit`.
4. **Чужие ветки не трогать:** не сливать, не перебазировать, не
   откатывать и не «чинить» их коммиты. Своя задача — свой worktree от
   `main`; в чужой worktree не заходить.
5. **Проверка перед сдачей:** `git diff --name-only main HEAD` — только
   свои пути; на площадке `md5sum` изменённых файлов совпадает с локальными,
   а всё остальное — с резервной копией.

## Живые ссылки, а не описания — правило №5

Владелец 12.09.2026: **«требование генерировать тут всегда и полные живые
линки и ссылки, по которым можно сразу всё открыть в браузере»**.

Ответ вроде «страница бэк-офиса», «раздел Номенклатура», «документ
ANGAJATI.md» заставляет человека искать адрес руками. Поэтому:

1. **Каждый упомянутый экран, документ или API — полным адресом с
   `https://`**, кликабельным: `https://officeplus.md/UNA.md/orasldev/crm/`,
   а не `/UNA.md/orasldev/crm/` и не «страница CRM».
2. **Если контуров два — давать оба**, подписав какой есть какой:
   officeplus.md (боевой) и nufarul.eminescu.md (тестовый, та же база
   Oracle). Внутренние площадки (`192.168.0.250:8001`) — отдельной строкой,
   с пометкой «только через VPN93».
3. **Ссылка обязана открываться.** Перед тем как её отдать, проверить
   код ответа (`curl -s -o /dev/null -w '%{http_code}'`, для страниц под
   логином `302 → /login` тоже считается живой). Мёртвых ссылок не давать.
4. **Якорь — до нужного места**, а не до начала простыни:
   `…/GHID_CRM.html#comenzi`, `…/crm/#employees`.
5. **То же в документах и на плитках:** в `.md`/`.html`, которые пишет
   агент, ставить полные адреса (или относительные, если файл сам живёт
   в портале — но тогда рядом блок «Adrese live» с полными). На плитках
   хаба у каждой возможности — ссылка на её раздел документации.
6. **В ответе человеку** — тот же порядок: сначала что сделано, потом
   список живых адресов, по которым это видно.

## Ветки, worktree, новые проекты — концепция

Полностью: [`docs/GIT_WORKFLOW.md`](docs/GIT_WORKFLOW.md); короткая версия
для любого агента: [`AGENTS.md`](AGENTS.md). Суть:

1. **Одна задача = один worktree = одна ветка.** В общем каталоге
   `/Users/pt/Projects.AI/Artgranit` — только `main`, только merge и деплой.
   Перед первым действием: `git branch --show-current && git worktree list`.
   Если ты в общем каталоге не на `main` — это чужая ветка, работать нельзя.
2. **Hook коммитит и пушит в репозиторий изменённого файла**, а не в
   фиксированный каталог — работает и в worktree.
3. **Общие файлы — только точечный `Edit`** после `git log -3 -- <файл>`;
   `Write` целиком общего файла запрещён.
4. **Новый проект-модуль — одной командой:**
   `python scripts/new_module.py <ключ> "<Название>"` или `/new-module`.
