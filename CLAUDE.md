...
## ⛔ Заморозка работ по архивам до среды 2026-08-05

Указание владельца проекта от 2026-07-31. **До среды 5 августа 2026 никаких работ по
архивам и резервному копированию.**

Под запретом до этой даты:

1. Изменения в `backup_local.sh`, `backup_remote.sh` и в шагах 1–2 `deploy_to_remote.sh`
   (резервная копия на сервере и локальный архив на 2,2 ГБ перед каждым деплоем).
2. Чистка, перенос и переупаковка каталога `backups/` (41 ГБ) и корневых
   `Artgranit_deploy_*.tar.gz`.
3. Любая оптимизация размера деплой-архива сверх уже сделанной.

Что уже сделано и менять не нужно: из архива деплоя исключены `AccountingDemoXcode`
(600 МБ) и `.zip` к нему, `.claude` (92 МБ), `.superpowers`, а также старые `venv.*`.
Благодаря этому архив уменьшился с 480 МБ до 37 МБ — до этого `scp` падал по таймауту
(`Operation timed out` / `Broken pipe`), деплой обрывался на шаге 4. Прод при таком обрыве
не страдает: `set -e` останавливает скрипт ДО каких-либо изменений на сервере.
