...
async function loadCardProdInfo(cod) {
  ...
  if (!box) return;
  // RO: atributele web multilingve (TMS_MPT_WEBATTR, BLOB = original cu
  //     diacritice) + comentariile clientilor. Copiile de cautare fara
  //     diacritice le regenereaza triggerul — aici se scrie DOAR BLOB.
  const [r, wr] = await Promise.all([
    apiGet('/api/biro26/shop/product/' + cod),
    apiGet('/api/biro26/webattr/' + cod)]);
  const d = (r.success && r.data) || {descriere: '', comments: []};
  const wa = (wr.success && wr.data) || {};
  window['WA_' + cod] = wa;
  const tabBtn = (L, on) =>
    '<button class="btn-sm wa-tab-' + cod + '" data-l="' + L + '" ' +
    'style="' + (on ? 'background:#1d4ed8;color:#fff;border-color:#1d4ed8' : '') + '" ' +
    'onclick="waTab(' + cod + ',\'' + L + '\')">' + L.toUpperCase() +
    ((wa[L] && (wa[L].descriere || wa[L].denum_full)) ? ' ●' : '') + '</button>';
  box.innerHTML =
    '<div class="detail-section-title">Atribute web (magazin) · Веб-атрибуты (магазин)</div>' +
    '<div class="muted" style="font-size:11.5px;margin:2px 0 6px">Diacriticele se păstrează ' +
    'integral (ăâîșț) — originalul intră în BLOB, copiile de căutare se generează automat.</div>' +
    '<div class="btn-row" style="margin-bottom:6px">' +
    tabBtn('ro', true) + tabBtn('ru', false) + tabBtn('en', false) + '</div>' +
    '<input id="wa-full-' + cod + '" placeholder="Denumire completă · Полное название" ' +
    'style="width:100%;padding:8px;border:1px solid #e2e8f0;border-radius:8px;' +
    'font:inherit;font-size:13px;margin-bottom:6px" value="' +
    escapeHtml((wa.ro || {}).denum_full || '') + '">' +
    '<textarea id="wa-desc-' + cod + '" placeholder="Descriere · Описание" ' +
    'style="width:100%;min-height:90px;padding:8px;' +
    'border:1px solid #e2e8f0;border-radius:8px;font:inherit;font-size:13px">' +
    escapeHtml((wa.ro || {}).descriere || d.descriere || '') + '</textarea>' +
    '<div class="btn-row" style="margin:6px 0 10px"><button class="btn primary" ' +
    'onclick="saveWebattr(' + cod + ')">💾 Salvează (limba curentă)</button>' +
    '<span class="muted" id="wa-lang-' + cod + '" data-l="ro" style="font-size:12px;' +
    'align-self:center">limba: RO</span></div>' +
