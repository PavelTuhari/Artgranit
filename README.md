...
8.  **TBControl (эксплуатация Front Office / POS / Self-Service / Android):** operations-центр торговой сети по ТЗ `docs/TBControl/TECHNICAL-OPS.md` — «мониторим не компьютер, а способность магазина продавать».
    ...
    *   **Эмулятор / Zabbix** (`tbc_emulator.py`, кнопка «🧪 Эмулятор» в UI): фоновый прогон всех 10 сценариев по кругу (сбои → восстановление) или подключение к реальному Zabbix (JSON-RPC, problem.get → события P1–P4, хост → устройство, автозакрытие решённых); API `/api/tbc/emulator/start|stop|status`, конфиг в `TBC_SETTINGS`.
    ...
    *   **Источники мониторинга** (`TBC_SOURCES`, панель «🛰 Источники»): `emulator`, `zabbix` (реальный Zabbix 3.x–7.x) и **`unisim_cassa` — порт логики боевого дашборда unisim-dashboard.una.md**: реестр касс `ybmb_dif_cassa` + проверка каждой кассы по Oracle DB Link (`tms_init_params@<PREFIX>.WORLD`, ServerID), агрегат магазинов из `tms_univers` (FUNCTIONAL при online ≥ 60%), снимок в `TBC_CASSA_STATE`, панель «🧾 Кассы UaMenu», переходы ONLINE↔OFFLINE порождают/закрывают события P3. Доступ к Oracle 11g `cloudbd` — через существующий thick subprocess-воркер, thin-контур production не затрагивается.
    *   **Сервисы Zabbix через mTLS** (`zabbix_svc`, `TBC_SERVICES`): мониторинг всех сервисов, подключённых к Zabbix сети Unisim (web/ssl/db/mail/network/server), с доступом **строго по клиентскому сертификату компьютера оператора** — nginx-шлюз `192.168.0.148:8443` с `ssl_verify_client on` и сверкой отпечатка, приватный ключ только в macOS Keychain, сверка SHA-256 в приложении; с других машин (включая production) опрос невозможен. Документ: `docs/TBControl/MTLS_SOURCE.md`.
    *   **Cassa Monitor** (`/UNA.md/orasldev/tbcontrol/cassa`) — отдельный полноэкранный дашборд касс, аналог `unisim-dashboard.una.md`: KPI, поиск/фильтры/сортировка, блок проблемных касс, карточки магазинов с раскрытием DB Links, автообновление. Весь модуль (панель, дашборд, презентация, документация) адаптирован под телефон: выдвижное меню, скроллируемые таблицы, свайп в презентации.
...
9.  **Планограммы (Planograms):** управление выкладкой товара и торговым пространством магазина — трёхъязычный модуль (**RU / RO / EN**).
    ...
    *   **Тестовое окружение сети магазинов:** тестовые данные изолированы в именованных наборах (`PLG_DATASETS`, набор `DEMO` защищён); пять алгоритмов генерации (`network`, `assortment`, `events`, `demand`, `traffic`) с запуском и прогрессом в админке. Модель спроса: базовый уровень × годовая сезонность категории × недельный профиль × тренд × promo-аплифт × шум, поверх — out-of-stock и (s,S) политика запаса. Проверено: 10 магазинов × 400 SKU × 365 дней = 911 373 строки за ~5 минут.
    *   **Прогноз заказов:** четыре алгоритма (`sma`, `ses`, `holt_winters`, `promo_reg`) в чистом Python/SQL без новых зависимостей; модель = алгоритм + конфигурация (`PLG_FCT_MODELS`), форма конфигуратора строится автоматически из `PARAMS_SCHEMA`. Режим `backtest` считает MAPE/MAE/RMSE/bias и сравнивает модели на одних данных; прогноз превращается в заказ через страховой запас по уровню сервиса и округление до короба.
    *   **Логистика завоза:** схема «РЦ + прямые поставки», три плеча (поставщик → РЦ, РЦ → магазин по дням маршрута, прямой завоз фреша). Диаграмма Ганта своим SVG: строка — машина / магазин / док / поставщик, полоса — плановое окно, линия под ней — фактическое, красная засечка — срыв окна. Генератор разводит окна так, чтобы на одном доке и одной машине рейсы не пересекались.
    *   **Взаимоотношения с поставщиками:** карточка, контактные лица по ролям, договоры (отсрочка, ретро-бонус, маркетинговый взнос, Incoterms), товарные группы работы. Двудольный граф «поставщик ↔ товарная группа», толщина связи — годовой оборот.
    *   **Анализ конкурентов:** сети с позиционированием, мониторинг их цен по нашим SKU (ценовой индекс = их цена / наша × 100), их поставщики с пометкой пересечения с нашей базой. Тепловая матрица «конкурент × товарная группа» + импорт замеров цен из CSV.
    *   **Рынки других стран:** макропоказатели шести рынков и схожие торговые сети; пузырьковая диаграмма (магазины × средний чек, размер — выручка) с отдельной точкой «наша сеть».
    *   **Фреш-контур:** отдельный алгоритм заказа скоропортящегося товара. Уровень запаса считается не по нормативу, а по критическому отношению newsvendor (упущенная маржа против стоимости списания), покрытие — по календарю маршрута поставки, а не «плечом в днях». Два маршрута — через распределительный центр и прямой поставкой — это ДАННЫЕ (`PLG_FRESH_ROUTES`), а не два разных алгоритма. Заказ ограничивается остаточным сроком годности с явной пометкой риска, презентационный минимум ограничен спросом за срок годности. В результате рядом с количеством стоят покрытие, ближайшая поставка и ожидаемое списание.
    *   **Мобильное приложение и голосовой заказ из зала:** авторизация по токену устройства (в базе SHA-256), одноразовый код сопряжения, лимит суммы на устройство, мгновенный отзыв. Распознавание речи выполняет само устройство — на сервер приходит текст, аудио не хранится. Разбор детерминированный (числительные, единицы, команды на трёх языках), непонятая позиция не угадывается: помечается, блокирует отправку и после ручного выбора пополняет речевой словарь. Каждая фраза с разбором пишется в `PLG_VOICE_LOG`.
    *   **Демо-режим:** модуль открывается без входа в режиме «только чтение» — чтобы ссылки из презентации и документации вели в работающую систему. Запись закрыта на сервере (`before_request`-хук отклоняет любой не-GET к `/api/plg/*` без сессии), а не только скрытием кнопок.
    *   **ИИ-мониторинг продаж:** семь детекторов, включая **векторный выброс** на Oracle 26ai (вектор поведения SKU в колонке `VECTOR(12)`, расстояния считает `VECTOR_DISTANCE` в базе, похожие SKU — по HNSW-индексу); остальные шесть детекторов (риск out-of-stock, всплеск/провал против недельной базы, риск списаний фреша, дрейф смещения модели, мёртвый запас) с трёхъязычными сообщениями и переходом в нужный раздел; витрина признаков по каждому SKU (24 колонки: уровни, CV, тренд, промо-аплифт, OOS, ABC/XYZ) с выгрузкой CSV/JSON для обучения моделей.
    *   **Автозаказ с корректировкой на лету:** рекомендация модели и решение закупщика в одной строке, правка сохраняется мгновенно, модельная цифра фиксируется отдельно (расхождение — обучающий сигнал). Печатный пакет документов заказа по поставщикам.
    *   **Заказы импорта:** 11 этапов от контракта до склада, задержки = факт − план с обязательной причиной, упреждающий чек-лист из 10 документов с дедлайнами от плановой даты границы (включая локализованные этикетки RO/RU), коды ТН ВЭД в позициях, статистика «где теряются дни» по этапам и причинам, печатный пакет для брокера. Три BPMN-схемы: заказ импорта, растаможка, подготовка документов.
    *   **Топливный контур сети АЗС (`PECO_*`):** раздел `#fuel` с картой станций (Leaflet + OpenStreetMap, библиотека вендорится локально), автозаказом по резервуарам и двухэшелонной схемой «импорт → нефтебаза → АЗС». Заказ ограничен свободной ёмкостью **на момент прихода бензовоза** и подбирается комбинацией секций цистерны с округлением вниз; сверху — экономический потолок покрытия. Четыре алгоритма прогноза отпуска на чистом Python (`models/peco_forecast.py`): Theta, Croston + SBA, конформный страховой запас без гипотезы нормальности, градиентный бустинг на деревьях; выбор — по backtest на реальной истории (`PECO_FCT_BACKTESTS`), метрика — ошибка накопленного спроса за горизонт завоза. Путь снабжения выбирается точным потоком минимальной стоимости (`models/peco_sourcing.py`) по комбинациям импорт / внутренний рынок ↔ своя или партнёрская нефтебаза ↔ АЗС; цена литра «до бака» включает транспорт, перевалку, пошлину и стоимость денег на плече, опоздание удорожает путь, а не запрещает его. Рейсы бензовозов по секциям и приём GPS-телеметрии от провайдера на аутсорсе по токену (`POST /api/peco/gps/ping`).
    *   DDL: `sql/80..83_plg_*.sql` (модуль), `84..86` (генератор и прогноз), `87..90` (логистика, партнёры, представления, словарь), `91..92` (бизнес-процессы), `93` (фреш), `94` (мобильный контур), `95` (словарь фреша), `96` (ИИ-мониторинг и корректировки), `97` (импорт), `98` (словарь новых разделов), `99` (векторный контур); топливо — `100..104`, `106..107` (снабжение), `109` (словарь топлива), `110` (алгоритмы и пути), `111` (демо-пути), `112` (словарь алгоритмов); ставится отдельно: `python deploy_oracle_objects.py --only plg_ peco_`.
    *   **Публичная документация модуля** отдаётся самим приложением: хаб `/UNA.md/orasldev/planograms/docs`, презентация `/UNA.md/orasldev/planograms/presentation`. Руководство пользователя, план презентации и слайды открыты без входа; техническое описание и инструкция по установке — за входом (содержат пути на сервере, размещение wallet и перечень ключей окружения). Доступ управляется флагом `public` в реестре `PLG_DOCS` в `app.py`.
    *   Исходники документации: `docs/Planograms/` — `PLANOGRAMS_MODULE.md` (техническое описание), `INSTALL.md` (установка и скрипты), `USER_GUIDE.md` (руководство пользователя), `AUTO_ORDER_GUIDE.md` (методичка по алгоритмам автозаказа со скриншотами, дорожной картой ИИ и сравнением подходов), `FUEL_LOGISTICS.md` (топливная логистика: расчёт по резервуару, четыре алгоритма прогноза, выбор пути снабжения, рейсы и GPS), `PRESENTATION_PLAN.md` + `presentation.html` (презентация).

10. **SDA (Sistemul Depozit-Returnare — regim tichete/Ambalaje SD):** clasificarea rețelei de retail, registrul ambalajelor și dosarul de înregistrare la Administrator conform pct. 78.
    *   Маршрут `/UNA.md/orasldev/sda`, API `/api/sda/*`, Oracle-объекты с префиксом `SDA_`.
    *   Таблицы `SDA_PARTIC`/`SDA_UNIT` хранятся в облачном ADB платформы, а не в ERP клиента; номенклатура OfficePlus подключена только на чтение через `biro26_db`.

11. **Карта системы и меню без ручных списков:** боковая панель портала и страница `/UNA.md/orasldev/modules` собираются при запуске из карты маршрутов Flask (`models/module_registry.py`), а не из списка в шаблоне. Любая страница под `/UNA.md/orasldev/…` попадает в меню сама — раньше в панели было 5 пунктов при 99 работающих страницах, и Biro26, Планограммы, ServOuts26, Colass, Nufărul и DECOR были невидимы из портала. Манифест `modules/<ключ>/module.json` задаёт название на трёх языках, иконку, порядок и подписи страниц, но **спрятать модуль из меню не может**; модуль без манифеста показан с пометкой «без манифеста», манифест без живых маршрутов — красной карточкой. Реестр документации работает так же: `docs/<Модуль>/*.md` подхватываются автоматически (`models/doc_registry.py`), `docs.json` задаёт заголовки и флаг `public` (по умолчанию документ виден в хабе, но закрыт входом). Внутри SPA-модуля новая панель `panel-<id>` подхватывается навигацией (`syncNavWithPanels`). Подробности: `docs/MODULE_REGISTRY.md`, JSON — `/api/modules`.
...
Приложение управляется через `systemd`. Единственный правильный способ перезапуска:

```bash
sudo systemctl restart artgranit
```

Или одной командой по SSH с локальной машины:

```bash
ssh -i ~/Downloads/ssh-key-2024-10-06.key ubuntu@92.5.3.187 'sudo systemctl restart artgranit'
```
...
**Не использовать** `pkill` + `nohup` или `full_restart.sh` — это уводит процесс из-под systemd, и приложение не поднимется автоматически после ребута сервера.
