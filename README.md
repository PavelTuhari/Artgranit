...
9a. **Общее ядро и изолированные модули (правило №1):** модуль портала — самодостаточный пакет `modules/<ключ>/`, который ядро (`core/module_loader.py`) находит, проверяет и подключает под `/UNA.md/orasldev/<ключ>` само. **В общем коде модуль не оставляет ничего:** ни строки в `app.py`, ни записи в `deploy_oracle_objects.py`, ни файла в `models/`, `controllers/`, `templates/`, `sql/`. Адреса внутри модуля пишутся без префикса — его подставляет ядро, поэтому модуль физически не может занять чужой адрес; на время импорта ядро закрывает ему `app.add_url_rule` и `app.register_blueprint`. Сбой одного модуля не роняет портал: битый импорт попадает в отчёт `app.extensions["module_loader"].as_dict()`, остальные работают. Смысл — скорость: несколько ИИ и сессий добавляют модули параллельно и не конфликтуют, потому что общий файл никто не правит. Образцы: `modules/seoforge/` и `modules/sda/` (последний везёт с собой и DDL `sql/117_sda_tables.sql`, и собственный установщик `scripts/sda_deploy.py`). Контракт и гарантии: `docs/CORE_MODULES.md`, правила для разработчика — `CLAUDE.md`.

10. **Карта системы и меню без ручных списков:** боковая панель портала и страница `/UNA.md/orasldev/modules` собираются при запуске из карты маршрутов Flask (`models/module_registry.py`), а не из списка в шаблоне. Любая страница под `/UNA.md/orasldev/…` попадает в меню сама — раньше в панели было 5 пунктов при 99 работающих страницах, и Biro26, Планограммы, ServOuts26, Colass, Nufărul и DECOR были невидимы из портала. Манифест `modules/<ключ>/module.json` задаёт название на трёх языках, иконку, порядок и подписи страниц, но **спрятать модуль из меню не может**; модуль без манифеста показан с пометкой «без манифеста», манифест без живых маршрутов — красной карточкой. Реестр документации работает так же: `docs/<Модуль>/*.md` подхватываются автоматически (`models/doc_registry.py`), `docs.json` задаёт заголовки и флаг `public` (по умолчанию документ виден в хабе, но закрыт входом). Внутри SPA-модуля новая панель `panel-<id>` подхватывается навигацией (`syncNavWithPanels`). Подробности: `docs/MODULE_REGISTRY.md`, JSON — `/api/modules`.
