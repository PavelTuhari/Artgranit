...
9.  **Планограммы (Planograms):** управление выкладкой товара и торговым пространством магазина — трёхъязычный модуль (**RU / RO / EN**).
    ...
    *   **Автозаказ топлива (сеть АЗС):** раздел `#fuel` поверх данных модуля PECO. Двухэшелонная схема «импорт → нефтебаза → АЗС». Объём заказа ограничен свободной ёмкостью резервуара **на момент прихода** бензовоза и подбирается комбинацией секций цистерны (одна секция — один вид топлива); целевой уровень ограничен и физически, и экономически (потолок запаса в сутках). Карта станций на Leaflet + OSM с перетаскиванием маркера и геокодированием по адресу — координаты сохраняются в Oracle, а не скачиваются файлом. Планирование рейсов бензовозов, приём GPS-телеметрии от внешнего провайдера по токену и детекторы: стоянка вне коридора маршрута, съезд, срыв пломбы, превышение скорости.
    *   DDL: `sql/80..83_plg_*.sql` (модуль), `84..86` (генератор и прогноз), `87..90` (логистика, партнёры, представления, словарь), `91..92` (бизнес-процессы), `93` (фреш), `94` (мобильный контур), `95` (словарь фреша), `96` (ИИ-мониторинг и корректировки), `97` (импорт), `98` (словарь новых разделов); ставится отдельно: `python deploy_oracle_objects.py --only plg_`.
...
по трём независимым расхождениям (по счётчику, по кассе, по каждому резервуару отдельно).
