...
Один из ключевых вопросов, возникающих у нового разработчика: «Как приложение подключается к базе данных?». В случае обычного PostgreSQL или MySQL всё просто: host, port, user, password -- и готово. С Oracle Autonomous Database (ADB) всё несколько сложнее, но зато значительно безопаснее.

ADB в Oracle Cloud работает через mTLS (mutual TLS) -- это означает, что не только клиент проверяет сертификат сервера, но и сервер проверяет сертификат клиента. Этот клиентский сертификат и ключи упакованы в так называемый **Oracle Wallet** -- папку с набором файлов (`ewallet.pem`, `cwallet.sso`, `keystore.jks` и др.). Wallet скачивается из OCI Console один раз, и далее используется для всех подключений. Без wallet подключиться к ADB невозможно -- никакой пароль не поможет.
...
APEX и ADB -- это один и тот же сервис: APEX работает внутри ADB, подключаться отдельно не нужно. APEX Workspaces создаются администратором внутри той же БД. Частый вопрос: «Нужен ли отдельный аккаунт или подключение для APEX?» Ответ: нет. APEX -- это компонент, установленный внутри ADB. Он работает на том же сервере, с теми же таблицами, в том же schema. Если вы создаёте таблицу через Python-приложение, она видна в APEX, и наоборот.

### 3.2 Структура подключения

Разберём механизм подключения подробно. Python-библиотека `oracledb` (начиная с версии 2.0) работает в *thin mode* -- это значит, что для подключения не нужен установленный Oracle Client. Библиотека сама умеет парсить файлы wallet, устанавливать TLS-соединение и обмениваться данными по протоколу Oracle Net. Подключение происходит в `models/database.py` -- класс `DatabaseConnection` сначала проверяет, что `WALLET_DIR` указывает на существующую папку, затем определяет метод подключения: если в wallet есть `ewallet.pem`, используется `wallet_location + wallet_password`; если нет -- `config_dir` (для SSO-wallet без пароля). Соединения управляются через connection pool (2-10 соединений), чтобы не открывать TLS-handshake на каждый HTTP-запрос.
...
Все секреты подключения хранятся в файле `.env` (который **не коммитится в Git**). Файл `.env.example` содержит шаблон с описаниями всех переменных. Обратите внимание на `CONNECT_STRING` -- это полный TNS descriptor, а не короткий alias. Полный descriptor надёжнее, т.к. не зависит от содержимого `tnsnames.ora` в wallet и содержит всё необходимое для подключения: хост, порт, имя сервиса, параметры retry и TLS.
...
Этот раздел -- полная инструкция для разработчика, который получил доступ к репозиторию и хочет запустить проект на своей машине. Процедура спроектирована так, чтобы от момента `git clone` до работающего приложения прошло не более 10-15 минут. Единственная задержка -- получение Oracle Wallet у администратора, но это одноразовая операция.

Важно понимать логику разделения: Git хранит *код и DDL-скрипты* (всё, что можно безопасно разделить с командой). Wallet, пароли, API-ключи -- это *секреты*, которые передаются отдельно и никогда не попадают в репозиторий. Oracle-объекты (таблицы, вью, пакеты) создаются скриптом `deploy_oracle_objects.py` из DDL-файлов -- они тоже в Git, поэтому любой разработчик может воссоздать полную схему БД с нуля.
...
Все SQL-скрипты для создания объектов Oracle находятся в `sql/` и **отслеживаются в Git**. Это одно из ключевых правил проекта: DDL -- это часть кода, и она должна версионироваться наравне с Python-файлами.

Скрипты пронумерованы в порядке выполнения: сначала таблицы (они создают структуры), затем представления (зависят от таблиц), затем триггеры и пакеты (зависят от таблиц), и в конце -- демо-данные (INSERT, который зависит от всех предыдущих объектов). Нумерация не непрерывная: между группами оставлены пробелы (например, BUS: 01-04, CRED: 05-12, NUF: 14-22, DIGI: 20-23, DECOR: 24, CLS: 25-32, AGRO: 35-38), чтобы можно было добавлять скрипты внутри группы без перенумерации.

Скрипт `deploy_oracle_objects.py` умеет разбирать SQL*Plus-синтаксис (разделитель `/` между блоками), определять PL/SQL-блоки (пакеты, триггеры) и DDL/DML-команды, правильно обрабатывать строковые литералы с одинарными кавычками. Если таблица уже существует, CREATE получит ошибку ORA-00955, которая будет залогирована, но не остановит выполнение -- это нормальное поведение для идемпотентного deploy.
...
Deploy в Artgranit разделён на два независимых шага: *кодовый deploy* (обновление Python-кода, шаблонов, статики) и *DDL deploy* (создание/обновление Oracle-объектов). Это принципиальное решение: кодовый deploy происходит часто (при каждом обновлении), а DDL deploy -- только когда добавляются новые таблицы или меняется схема. Поскольку все окружения (local и remote) работают с *одной и той же* Oracle ADB, DDL достаточно выполнить один раз с любой машины.

Deploy реализован «по-старинке» -- через tar.gz + SSH/SCP, без Docker, Kubernetes или CI/CD. Это сознательный выбор для проекта текущего масштаба: один remote-сервер, один разработчик, minimal overhead. Скрипт `deploy_to_remote.sh` выполняет всю работу одной командой, но при этом надёжно сохраняет `.env` и wallet.
...
Локальная разработка ведётся на macOS. Приложение запускается на порту 3003 (чтобы не конфликтовать с remote на порту 8000 и с другими локальными сервисами). Скрипт `run_local.sh` инкапсулирует всю подготовку: создание виртуального окружения, установку зависимостей, настройку переменных окружения. При повторных запусках он пропускает уже выполненные шаги, так что запуск занимает несколько секунд.

Приложение слушает на `0.0.0.0` (а не на `127.0.0.1`), чтобы к нему можно было обращаться не только через `localhost`, но и по IP-адресу локальной сети -- например, для тестирования с мобильного устройства или с другого компьютера в той же сети.
...
Это, пожалуй, самый важный раздел документации. Он описывает полный цикл создания нового модуля -- от выбора имени до final checklist перед merge. Инструкция написана так, чтобы быть одинаково полезной и для разработчика, читающего её глазами, и для AI-агента (Claude Code, Cursor, GitHub Copilot), выполняющего задачу автоматически.

Каждый шаг содержит не только «что делать», но и «почему именно так» и «как проверить, что сделано правильно». Порядок шагов имеет значение: DDL создаётся *до* модели, модель -- *до* контроллера, контроллер -- *до* маршрутов. Каждый последующий слой зависит от предыдущего.

Аналогия с C++ Builder: если вы когда-либо создавали новый модуль данных (TDataModule) с набором TTable/TQuery, потом форму (TForm) с DBGrid и привязкой к источнику данных, и наконец регистрировали форму в главном меню -- здесь та же самая последовательность, только вместо TTable у нас Oracle DDL, вместо TDataModule -- Python model, вместо TForm -- HTML template, а вместо MainMenu -- Shell projects.

### 8.1 Выбрать префикс

Выбрать короткий (3-5 символов) уникальный префикс. Все Oracle-объекты модуля именуются `PREFIX_*`.

Выбор префикса -- это первое и одно из самых важных решений при создании модуля. Префикс должен быть коротким (3-5 символов), уникальным и интуитивно понятным. Он будет использоваться *повсюду*: в именах таблиц, sequences, triggers, views, PL/SQL-пакетов. Хороший префикс -- это аббревиатура предметной области: `AGRO` для агро-модуля, `CLS` для Colass, `CRED` для кредитов. Плохой префикс -- это что-то слишком общее (`APP_`, `DATA_`) или слишком длинное (`LOGISTICS_WAREHOUSE_`).

Почему это важно: когда в одной Oracle-схеме живут 150+ объектов от 9 модулей, единственный способ быстро понять, кому принадлежит таблица -- это посмотреть на её префикс. Команда `SELECT OBJECT_NAME FROM USER_OBJECTS WHERE OBJECT_NAME LIKE 'AGRO_%'` мгновенно покажет все 36 объектов AGRO-модуля.
...
DDL -- это «фундамент» модуля. Без правильной схемы данных всё остальное развалится. Обратите внимание на структуру шаблона ниже: сначала sequences (генераторы ID), затем master data (справочники), затем documents (документы-шапки), затем document items (строки документов), и наконец event log. Это классическая последовательность для любой учётной системы -- тот же паттерн, что и в Oracle E-Business Suite, SAP или 1С.

Каждая таблица имеет поля `CREATED_AT` и `UPDATED_AT` с `DEFAULT CURRENT_TIMESTAMP` -- это обеспечивает автоматическую метку времени создания без дополнительного кода. Флаг `IS_ACTIVE CHAR(1) DEFAULT 'Y'` с CHECK-constraint -- стандарт проекта для «мягкого удаления»: записи не удаляются физически, а деактивируются. Trigger auto-ID (`BEFORE INSERT ... WHEN (NEW.ID IS NULL)`) позволяет как автоматическую генерацию ID из sequence, так и явную вставку с заданным ID (полезно для миграций и seed-данных).
...
Контроллер -- это «мозг» модуля. Он принимает параметры из HTTP-запроса (через маршрут в `app.py`), вызывает методы модели, при необходимости трансформирует данные (агрегирует, фильтрует, вычисляет) и возвращает результат. Контроллер *не должен* содержать SQL-запросы напрямую -- это ответственность модели. И наоборот, модель *не должна* знать о HTTP, request, response -- это ответственность контроллера и маршрута. Такое разделение позволяет тестировать бизнес-логику отдельно от веб-слоя.
...
Маршруты регистрируются в `app.py` и делятся на два типа: **UI-маршруты** (возвращают HTML-страницу) и **API-маршруты** (возвращают JSON). UI-маршруты всегда начинаются с `/UNA.md/orasldev/`, API-маршруты -- с `/api/`. Каждый маршрут проверяет аутентификацию вызовом `AuthController.is_authenticated()` -- это стандартный boilerplate, который нужно копировать без изменений.
...
Этот раздел -- результат прожитого опыта. Каждый пункт ниже -- это ошибка, которая была допущена хотя бы один раз в истории проекта, потребовала значительных усилий для исправления, и теперь зафиксирована как запрет. Аналогия с C++ Builder: вы бы не стали хранить все записи TTable в одном TMemoField вместо нормальных полей -- по той же причине мы не храним бизнес-данные в JSON blob.
