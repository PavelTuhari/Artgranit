...
ALTER TABLE PLG_PRODUCTS ADD (
  IS_FRESH      NUMBER(1)    DEFAULT 0,     -- участвует в фреш-контуре заказа
  TEMP_REGIME   VARCHAR2(20) DEFAULT 'ambient',
  COST_PRICE    NUMBER(14,2),               -- закупочная цена, для стоимости списания
  SALVAGE_PCT   NUMBER(5,2)  DEFAULT 0,     -- % себестоимости, возвращаемый уценкой
  CASE_WEIGHT_KG NUMBER(10,3)               -- вес короба, для расчёта машин
);

ALTER TABLE PLG_PRODUCTS ADD CONSTRAINT CHK_PLG_PROD_FRESH CHECK (IS_FRESH IN (0,1));
ALTER TABLE PLG_PRODUCTS ADD CONSTRAINT FK_PLG_PROD_TEMP
...
CREATE TABLE PLG_FRESH_ROUTES (
  ...
  CONSTRAINT FK_PLG_FR_DS    FOREIGN KEY (DATASET_ID)  REFERENCES PLG_DATASETS(ID),
  CONSTRAINT FK_PLG_FR_STORE FOREIGN KEY (STORE_ID)    REFERENCES PLG_STORES(ID) ON DELETE CASCADE,
  CONSTRAINT FK_PLG_FR_CAT   FOREIGN KEY (CATEGORY_ID) REFERENCES PLG_CATEGORIES(ID),
  CONSTRAINT FK_PLG_FR_SUP   FOREIGN KEY (SUPPLIER_ID) REFERENCES PLG_SUPPLIERS(ID) ON DELETE CASCADE,
  CONSTRAINT FK_PLG_FR_DC    FOREIGN KEY (DC_ID)       REFERENCES PLG_DC(ID) ON DELETE CASCADE,
  CONSTRAINT CHK_PLG_FR_ROUTE  CHECK (ROUTE IN ('dc','direct')),
  CONSTRAINT CHK_PLG_FR_ACTIVE CHECK (IS_ACTIVE IN (0,1)),
  CONSTRAINT CHK_PLG_FR_LEAD   CHECK (LEAD_TIME_DAYS BETWEEN 0 AND 30)
);
/
