...
CREATE TABLE PECO_EMPLOYEES (
  ...
  CONSTRAINT CK_PECO_EMP_ACT CHECK (ACTIVE IN (0,1))
);
...
CREATE INDEX IX_PECO_EMP_ST     ON PECO_EMPLOYEES (STATION_ID);

-- Не более одной "действующей" (VALID_TO IS NULL) цены на станцию+вид топлива.
-- Функциональный индекс: для закрытых строк (VALID_TO не NULL) выражения дают NULL
-- и не участвуют в уникальности, так что история цен не ограничивается.
CREATE UNIQUE INDEX UX_PECO_PRICES_CURRENT ON PECO_PRICES (
  CASE WHEN VALID_TO IS NULL THEN STATION_ID  END,
  CASE WHEN VALID_TO IS NULL THEN GRADE_CODE END
);
